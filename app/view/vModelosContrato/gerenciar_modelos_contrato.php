<?php
// Conexão com MySQL
require_once 'C:/wamp64/www/projetos/crmx/db_conn/dbConn.php';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  if ($acao === 'excluir' && $id_modelo) {
    $conn->prepare("DELETE FROM modelos_contratos_clausulas WHERE id_modelo = ?")->execute([$id_modelo]);
    $conn->prepare("DELETE FROM modelos_contratos WHERE id_modelo = ?")->execute([$id_modelo]);
    $mensagem = "✅ Modelo excluído com sucesso.";
  }

  if (($acao === 'criar' || $acao === 'salvar') && $nome && $ordem) {
    if ($acao === 'criar') {
      $stmt = $conn->prepare("SELECT id_modelo FROM modelos_contratos WHERE nome_modelo = ?");
      $stmt->execute([$nome]);
      if ($stmt->rowCount() > 0) {
        $id_modelo = $stmt->fetchColumn();
      } else {
        $conn->prepare("INSERT INTO modelos_contratos (nome_modelo) VALUES (?)")->execute([$nome]);
        $id_modelo = $conn->lastInsertId();
      }
    }

    if ($acao === 'salvar' && $id_modelo) {
      $conn->prepare("UPDATE modelos_contratos SET nome_modelo = ? WHERE id_modelo = ?")->execute([$nome, $id_modelo]);
      $conn->prepare("DELETE FROM modelos_contratos_clausulas WHERE id_modelo = ?")->execute([$id_modelo]);
    }

    $ids = explode(',', $ordem);
    $stmt = $conn->prepare("INSERT INTO modelos_contratos_clausulas (id_modelo, id_clausula, ordem) VALUES (?, ?, ?)");
...
//$clausulas = $conn->query("SELECT id, titulo, descricao FROM clausulas WHERE id_pai IS NULL ORDER BY id")->fetchAll();

// Carrega cláusulas principais
$clausulas = $conn->query("
  SELECT _id_clausula, titulo, nome_ref, tipo
  FROM clausulas
  WHERE tipo = 'clausula'
  ORDER BY titulo
")->fetchAll();
...
$subclausulas = $conn->query("
  SELECT s._id_clausula, s.titulo, s.nome_ref, p.titulo AS pai
  FROM clausulas s
  JOIN clausulas p ON s.id_pai = p._id_clausula
...
$itens = $conn->query("
  SELECT i._id_clausula, i.titulo, i.nome_ref, p.titulo AS pai
  FROM clausulas i
  JOIN clausulas p ON i.id_pai = p._id_clausula
  WHERE i.tipo = 'item'
  ORDER BY i.titulo
")->fetchAll();
...
<form method="post">
  ...
  <select id="select_clausula">
    ...
    <?php foreach ($clausulas as $c): ?>
      <option value="<?= $c['_id_clausula'] ?>" data-titulo="<?= htmlspecialchars($c['titulo']) ?>" data-descricao="" data-ref="<?= $c['nome_ref'] ?>">
  ...
  <select id="select_subclausula">
    ...
    <?php foreach ($subclausulas as $s): ?>
      <option value="<?= $s['_id_clausula'] ?>" data-titulo="<?= htmlspecialchars($s['titulo']) ?>" data-descricao="" data-ref="<?= $s['nome_ref'] ?>" data-pai="<?= htmlspecialchars($s['pai']) ?>">
  ...
  <select id="select_item">
    ...
    <?php foreach ($itens as $i): ?>
      <option value="<?= $i['_id_clausula'] ?>" data-titulo="<?= htmlspecialchars($i['titulo']) ?>" data-descricao="" data-ref="<?= $i['nome_ref'] ?>" data-pai="<?= htmlspecialchars($i['pai']) ?>">
...
// ###
  ...
  inputNome.addEventListener('input', () => {
    ...
    fetch(`api/apiRead/buscar_modelos.php?q=${encodeURIComponent(termo)}`)
      .then(r => r.json())
      .then(data => {
        sugestoes.innerHTML = '';
        data.forEach(item => {
          ...
          li.onclick = () => {
            fetch(`api/apiRead/carregar_modelo.php?id=${item.id_modelo}`)
              .then(r => r.json())
              .then(data => {
                ...
                data.clausulas.forEach(c => {
                  ...
                  div.setAttribute('draggable', 'true');
                  div.setAttribute('data-id', c._id_clausula);
