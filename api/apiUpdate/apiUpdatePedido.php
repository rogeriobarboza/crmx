...
try {
    ...
    $camposObrigatorios = ['id_pedido', '_id_contato', 'titulo_evento', 'data_reservada', 'valor_total'];
    ...
    $sql = "UPDATE pedidos SET 
        _id_contato = ?, nome_contato = ?, produto_servico = ?, seguimento = ?, 
        titulo_evento = ?, data_reservada = ?, descricao_pedido = ?, participantes = ?, 
        ...
        vencimento_mensal = ?, reserva_equipe = ?, estimativa_custo = ?, info_adicional = ?
        WHERE id_pedido = ?";

    $stmt = $conn->prepare($sql);
    $stmt->bind_param(
        "issssssssissdddsisssssdsi",
        $dados['_id_contato'],
        $dados['nome_contato'],
    ...
    if ($stmt->execute()) {
        if ($stmt->affected_rows > 0) {
            ...
            throw new Exception("Nenhum pedido foi encontrado com o ID: " . $dados['id_pedido']);
        }
    } else {
